...
pC <- ggplot() +
  ...
  labs(tag = "c",
       title = "Cross-species molecular bridges: human MR loci ↔ animal Sweet Trap homologs") +
  theme(plot.title = element_text(size = 6.5, face = "bold",
                                  margin = margin(b = 2)))
...
ggsave(
  filename = file.path(out_dir, "fig6_mr_layer_D.png"),
  plot     = fig6,
...
ggsave(
  filename = file.path(out_dir, "fig6_mr_layer_D.pdf"),
  plot     = fig6,
  width    = 180, height = 200, units = "mm",
  device   = cairo_pdf
)

message("Figure 6 saved: fig6_mr_layer_D.png + .pdf")
