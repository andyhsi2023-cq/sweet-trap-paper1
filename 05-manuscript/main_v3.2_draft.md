...
**Manuscript length:** ~4,900 words main text (Introduction + Results + Discussion); ~3,650 words Methods; 149-word Abstract; 6 main figures; 2 tables; Extended Data (3 items); extensive Supplementary Information.
**Data and code availability:** OSF repository `https://osf.io/ucxv7/` (deposited 2026-04-18); GitHub: `sweet-trap-multidomain`.

**Version tracking.** v3.2 applied three NHB Article structural fixes on top of v3.1: [DIFF-F4] Abstract compressed to ≤ 150 words per NHB Article hard limit; [DIFF-F5] former §2 "The Sweet Trap Theory" section dissolved into the Introduction as six narrative paragraphs with no subheadings (formal axiom/theorem statements retained in Methods §M1); [DIFF-F6] Discussion restructured as four continuous paragraphs without subheadings. [DIFF-F7] One Extended Data item (ED Table 1: A1–A4 axioms + T1–T4 theorems formal statements synopsis) added to support the compressed narrative exposition. **v3.3 (this version)** applies NHB pre-submission compliance fixes: [B1] figures renumbered to match first-citation order in the main text; [B2] Author Contributions rewritten in CRediT taxonomy; [B4] Ethics statement added; [W3] consolidated reference list (SI-split phrase removed); [W5] Inclusion & Ethics in Global Research statement added; [W7] funding wording brought to NHB house style; references 15 and 17 updated from placeholder to published sources; cover letter trimmed to ≤ 550 body words with explicit preprint disclosure. All empirical numbers, theorem content, and figures are preserved verbatim.

---

## Abstract

Moths pursue artificial lights; sea turtles eat plastic; humans aspirationally over-consume investment leverage, short-video, mortgages, alcohol. We introduce **Sweet Trap**: reward–fitness decoupling under aspirational endorsement. Four axioms define a signed wedge Δ_ST between perceived and fitness utilities; a central theorem derives the bound |Δb_signal|/|Δb_info| ≥ 1.5 on intervention asymmetry under dose-matched comparison. A four-layer test combines a 20-case animal meta-analysis (pooled Δ_ST = +0.645), 19 Mendelian-randomisation chains (BMI→T2D OR = 2.06), ISSP trajectories across 25 countries, and 3,000 pre-registered specifications. A pre-registered cross-level joint test gives β = +1.58, p = 0.019; median intervention-asymmetry ratio across six domains = 11×. An F1+F2 binary classifier validates at Cohen's κ = 1.00 on 18 blind-coded cases. Signal-redesign interventions structurally outperform information within Sweet Trap domains; not elsewhere.

---

<!-- [DIFF-F5] NHB Article style: Introduction has NO heading. First paragraph begins the manuscript body. Former §2 Theory section (v3.1, 1,115 words) is dissolved here into paragraphs ¶3–¶6. Formal axioms/theorems moved to Methods §M1 and ED Table 1. -->
...
<!-- [DIFF-F6] NHB Article style: Discussion has NO subheadings. Former §4.1 Scope/Limitations, §4.2 Policy, §4.3 Empirical agenda, §4.4 Closing are now four continuous paragraphs, each beginning with a clear topic sentence that carries the former subheading's semantic load. Content and citations preserved verbatim. -->

We presented an axiomatic theory of reward–fitness decoupling (Sweet Trap) and empirically validated its core claims across four evidence layers. The theory rests on four axioms (A1 ancestral calibration; A2 environmental decoupling; A3 endorsement inertia with A3.0 operational scope; A4 partial cost visibility with hyperbolic discount P1) and yields four theorems (T1 LSE stability; **T2 intervention asymmetry by ≥ (1 − *w*_max)/*w*_max ≥ 1.5**; T3 cross-species universality; T4 engineered escalation with basin-radius demoted to Observation 4.1). The keystone is Δ_ST = *U*_perc − E[*U*_fit ∣ *B*]. Of five theorem-derived predictions, one (P1) is strongly supported, one (P3) partially supported, one (P5) is supported as an empirical regularity consistent with T3, one (P4) is qualitatively consistent, and one (P2) awaits dedicated empirical test. Ten distinct scope conditions and limitations bound these claims and must be stated alongside them. A3 applies only within A3.0 scope (abandonment < 30% or revealed *w* < 0.45); agents outside are not Sweet Trap cases and the theory makes no claim about them. T2's dose-matching is operational (nudge-RCT reporting convention, Methods §M1), not axiomatic — a flagged boundary. Asymmetric RCT-to-field shrinkage *κ*_signal ≠ *κ*_info could narrow T2's field-realised margin; the data here are consistent with approximate symmetry but direct *κ*_signal estimation is future work. Eleven of nineteen MR chains are Steiger ✗ (§M4.3); we follow Hemani–Tilling–Davey-Smith 2017²⁶, addressing socially-stratified-GWAS architecture in Methods. Orthogonal global-health accounting (4.1–34.6 M DALYs/yr) is retained in Appendix H as descriptive, not primary. Layer B C12 is fragile at the CFPS headline and reported as directional. The three-layer P5 test yields *p* = 0.47 with the A+D subset (β = +1.58, *p* = 0.019) as pre-registered headline. The A-D rank correlation ρ = +1.00 on *n* = 2 is a geometric identity, not inferential evidence. Pig-butchering is a held-out secondary-source prediction. PUA is SI-only after a 0.5-step F2 disagreement. Layer D outcomes are Finnish-only; multi-ancestry replication would strengthen external validity.

These limitations notwithstanding, the theory has concrete policy purchase. T2 formalises why information campaigns systematically underperform in Sweet Trap domains and signal-redesign dominates — a pattern already visible in the public meta-analytic record (auto-enrolment C8; SSB taxation C11; commitment caps C12; LTV rules C13; alcohol price D_alcohol; platform cold-approach friction C_pig-butchering). The vaccine-hesitancy counter-example shows the claim is scope-restricted: where belief is the binding variable (F1 fails), information wins. This yields a **policy predictability property**: the classification step (F1 + F2) specifies in advance which intervention family will succeed in a given domain, rather than retrospectively rationalising observed policy outcomes. Five currently-contested domains (digital-platform regulation; short-video minor protection; LTV caps; commitment devices for gambling; algorithmic disclosure) are clear signal-redesign targets under the construct's scope. The framework therefore serves as an *ex ante* intervention selector, not a post-hoc reclassifier.
